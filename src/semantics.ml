...
let lookup_struct_field ~line env sname field_name errs_so_far =
  try
    ...
    | Environment.Struct_type (_, entries) ->
      (match List.find entries ~f:(fun e -> String.equal e.entry_name field_name) with
       ...
       | None -> (Type_system.Int, errs_so_far @ [Type_error (line, "struct " ^ sname ^ " has no field named " ^ field_name)]))
    | _ -> (Type_system.Int, errs_so_far @ [Type_error (line, sname ^ " is not a struct type")])
  with
  | Environment.Unbound_symbol msg -> (Type_system.Int, errs_so_far @ [Type_error (line, msg)])

let rec type_of_expr ~line env = function
  | Ast.Assign (a, b) ->
    let (type_a, errs_a) = type_of_expr ~line env a in
    let (type_b, errs_b) = type_of_expr ~line env b in
    let errs = errs_a @ errs_b in
    if not (Type_system.equal type_a type_b) then
      (type_a, errs @ [Type_error (line, "incompatible types for assignment operation")])
    else (type_a, errs)
  | Ast.Binary (op, a, b) ->
    let (type_a, errs_a) = type_of_expr ~line env a in
    let (type_b, errs_b) = type_of_expr ~line env b in
    ...
    (match op with
     ...
     | Ast.Add ->
       (match type_a, type_b with
        ...
        | _ ->
          if not (Type_system.equal type_a type_b) then (type_a, errs @ [Type_error (line, "incompatible types for binary operation")])
          else (type_a, errs))
     | Ast.Sub ->
       (match type_a, type_b with
        | Type_system.Pointer _, Type_system.Int -> (type_a, errs)
    ...
    (match op with
     ...
     | Ast.Deref ->
       (match type_a with
        ...
        | _ -> (type_a, errs_a @ [Type_error (line, "dereferencing a non-pointer type")])))
  | Ast.Postfix (_op, a) -> type_of_expr ~line env a
  | Ast.Index (a, b) ->
    let (type_a, errs_a) = type_of_expr ~line env a in
    let (_type_b, errs_b) = type_of_expr ~line env b in
    let errs = errs_a @ errs_b in
    (match type_a with
     | Type_system.Array (elem, _) -> (elem, errs)
     | Type_system.Pointer elem -> (elem, errs)
     | _ -> (type_a, errs @ [Type_error (line, "indexing a non-array type")]))
  | Ast.Field (a, name) ->
    let (type_a, errs_a) = type_of_expr ~line env a in
    (match type_a with
     | Type_system.Struct (sname, _) -> lookup_struct_field ~line env sname name errs_a
     | _ -> (Type_system.Int, errs_a @ [Type_error (line, "field access on non-struct type")]))
  | Ast.Ptr_field (a, name) ->
    let (type_a, errs_a) = type_of_expr ~line env a in
    (match type_a with
     | Type_system.Pointer (Type_system.Struct (sname, _)) -> lookup_struct_field ~line env sname name errs_a
     | _ -> (Type_system.Int, errs_a @ [Type_error (line, "-> on non-pointer-to-struct type")]))
  | Ast.Call (func, args) ->
    let args_results = List.map args ~f:(type_of_expr ~line env) in
          ...
          | Environment.Func { return_type; params } ->
            ...
            if n_args <> n_params then
              (return_type, errs_args @
                [Type_error (line, fname ^ ": expected " ^ Int.to_string n_params
              ...
              let mismatches = List.filter_mapi (List.zip_exn arg_types param_types)
                ~f:(fun _ (at, pt) ->
                  ...
                  else Some (Type_error (line, fname ^ ": argument type mismatch")))
              in
              (return_type, errs_args @ mismatches)
          | _ -> (Type_system.Int, errs_args @ [Type_error (line, fname ^ " is not a function")])
        with
        | Environment.Unbound_symbol msg -> (Type_system.Int, errs_args @ [Type_error (line, msg)]))
     | _ ->
       let (_type_f, errs_f) = type_of_expr ~line env func in
       (Type_system.Int, errs_f @ errs_args @ [Type_error (line, "indirect function call not supported")]))
  ...
  | Ast.Identifier name ->
    (try
     ...
     with
     | Environment.Unbound_symbol msg -> (Type_system.Int, [Type_error (line, msg)]))
  | Ast.Null -> (Type_system.Null, [])

(* --- Helpers for statement checking --- *)

let check_expr ~line env expr = snd (type_of_expr ~line env expr)

let rec check_type_exists ~line env = function
  | Type_system.Struct (name, _) ->
    (try
       match Environment.fetch_decl env name with
       | Environment.Struct_type _ -> []
       | _ -> [Type_error (line, name ^ " is not a struct type")]
     with Environment.Unbound_symbol _ ->
       [Type_error (line, "undefined struct type: " ^ name)])
  | Type_system.Pointer t | Type_system.Array (t, _) -> check_type_exists ~line env t
  | _ -> []
...
let check_redecl ~line env name =
  try ignore (Environment.fetch_decl env name);
    [Type_error (line, "redeclaration of " ^ name)]
  with Environment.Unbound_symbol _ -> []

(* Extract and register inline struct definitions from a type *)
let rec register_inline_structs ~line env = function
  | Type_system.Struct (name, Some entries) ->
    let (env, nested_errs) = List.fold entries ~init:(env, []) ~f:(fun (env, errs) entry ->
      let (env', new_errs) = register_inline_structs ~line env entry.Type_system.entry_type in
      (env', errs @ new_errs))
    in
    let redef_errs = check_redecl ~line env name in
  ...
  | Type_system.Pointer t | Type_system.Array (t, _) ->
    register_inline_structs ~line env t
  | _ -> (env, [])

(* --- Statement checking (internal, with return type context) --- *)

let rec check_stmt ~ret_type ~line env = function
  | Ast.Loc (l, stmt) -> check_stmt ~ret_type ~line:l env stmt

  | Ast.Global_decl decl ->
    let decl_val = Environment.decl_of_ast decl in
    let ty = Environment.type_of_decl decl_val in
    let (env, inline_errs) = register_inline_structs ~line env ty in
    let redef_errs = check_redecl ~line env decl.name in
    let type_errs = check_type_exists ~line env ty in
  ...
  | Ast.Local_decl decl ->
    ...
    (match Environment.declare_local env decl.name decl_val with
     ...
     | exception (Environment.Duplicate_declaration name) ->
       (env, inline_errs @ [Type_error (line, "redeclaration of " ^ name)] @ type_errs)
  ...
  | Ast.Func_def (fd, body) ->
    let ret = resolve_return_type fd in
    let (env, inline_errs) = register_inline_structs ~line env ret in
    let redef_errs = check_redecl ~line env fd.name in
    ...
    let (func_env, param_errs) =
      List.fold fd.params ~init:(func_env, []) ~f:(fun (e, errs) (p : Ast.decl_statement) ->
        ...
        | e' -> (e', errs)
        | exception (Environment.Duplicate_declaration name) ->
          (e, errs @ [Type_error (line, "redeclaration of parameter " ^ name)])
  ...
  | Ast.Expr expr ->
    let (_, errs) = type_of_expr ~line env expr in
    (env, errs)

  | Ast.Return expr ->
    let (t, errs) = type_of_expr ~line env expr in
    let ret_errs = match ret_type with
      | Some rt when not (Type_system.equal t rt) ->
        [Type_error (line, "return type mismatch: expected "
  ...
  | Ast.If (cond, then_, else_opt) ->
    let (_, ce) = type_of_expr ~line env cond in
    let (_, te) = check_stmt ~ret_type ~line env then_ in
    let ee = match else_opt with
      | Some s -> snd (check_stmt ~ret_type ~line env s)
      | None -> []
    in
    (env, ce @ te @ ee)
  ...
  | Ast.While (cond, body) ->
    let (_, ce) = type_of_expr ~line env cond in
    let (_, be) = check_stmt ~ret_type ~line env body in
    (env, ce @ be)

  | Ast.For (init, cond, step, body) ->
    let ie = match init with Some e -> snd (type_of_expr ~line env e) | None -> [] in
    let ce = match cond with Some e -> snd (type_of_expr ~line env e) | None -> [] in
    let se = match step with Some e -> snd (type_of_expr ~line env e) | None -> [] in
    let (_, be) = check_stmt ~ret_type ~line env body in
    (env, ie @ ce @ se @ be)
...
and check_stmts ~ret_type env stmts =
  List.fold stmts ~init:(env, []) ~f:(fun (e, errs) s ->
    let (e', new_errs) = check_stmt ~ret_type ~line:0 e s in
    (e', errs @ new_errs))

(* --- Public interface --- *)

let check_statement env stmt = check_stmt ~ret_type:None ~line:0 env stmt
